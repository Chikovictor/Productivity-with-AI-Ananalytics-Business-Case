#  📌Project Title  
**AI-Driven Business Case: Launching a Premium Used Car Dealership in Kenya**  
A productivity-focused data analytics project using AI to deliver an investor-ready business plan in under 60 minutes.  

<div style="border: 2px solid #16e01f; padding: 10px; display: inline-block; border-radius: 6px;">
  <img src="Business_Case.png" alt="Dashboard Screenshot" width="800"/>
</div>

##  🎯 Problem Statement  
Traditional market research and business planning takes weeks, slowing decision-making and innovation. This project demonstrates how AI and analytics can dramatically improve productivity in real-world business strategy.

##  🏢 Business Context  
**Company**: Nguvu Automotive Car
**Objective**: Launch a premium used-car dealership branch in Kenya
**Cities Evaluated**: Nairobi vs Mombasa
**Goal**: Produce a complete, investor-ready plan under 60 minutes


##  📊 Key Deliverables 

✔ Market analysis (Nairobi vs Mombasa)  
✔ Location recommendation backed by data  
✔ Revenue & demand insights  
✔ Investor pitch deck  
✔ Store vision & branding concept  
✔ Logo & visual identity  
✔ Final strategic recommendation  



##  🧠 Role of AI & Analytics  
AI was used to accelerate research, structure analysis, generate business insights, and rapidly produce professional business artifacts, demonstrating how data-driven tools can significantly improve productivity in analytics and strategy roles.

##  🛠 Tools & Skills Demonstrated  
+ Data Analysis & Business Intelligence  
+ AI-assisted research & decision-making  
+ Market comparison & strategic analysis
+ Business storytelling & visualization  
+ PowerPoint / Pitch Deck design
+ GitHub documentation & version control    

##  📁 Repository Structure  
├── data /               <!--Market & reference data-->  
├── analysis /           <!--City comparison & insights-->  
├── pitch-deck /         <!--Investor presentation-->  
├── branding /           <!--Logo & store vision-->  
├── visuals /            <!--Images used in the case-->  
└── README.md / 

##  📌 Key Outcome  
The project successfully delivered a complete, investor-ready business case within the target time, highlighting how AI can transform productivity in analytics, consulting, and business strategy roles.

##  🙋‍♂️ About Me

Computer Science undergraduate with strong interest in Data Analytics/Data Science, Business Intelligence, and AI-driven productivity solutions.

[LinkedIn](linkedin.com/in/victor-mwadzombo-95a026291/)
